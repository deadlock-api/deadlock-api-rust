...
fn build_ch_query(settings: BulkMatchMetadataQuery) -> APIResult<String> {
    ...
    if let Some(max_unix_timestamp) = settings.max_unix_timestamp {
        where_clauses.push(format!("start_time <= {}", max_unix_timestamp));
    }
    if let Some(min_match_id) = settings.min_match_id {
        where_clauses.push(format!("match_id >= {}", min_match_id));
    }
    if let Some(max_match_id) = settings.max_match_id {
        where_clauses.push(format!("match_id <= {}", max_match_id));
    }
    if let Some(min_duration_s) = settings.min_duration_s {
        where_clauses.push(format!("duration_s >= {}", min_duration_s));
    }
    if let Some(max_duration_s) = settings.max_duration_s {
        where_clauses.push(format!("duration_s <= {}", max_duration_s));
    }
    if let Some(min_average_badge) = settings.min_average_badge {
        where_clauses.push(format!("average_badge_team0 >= {}", min_average_badge));
        where_clauses.push(format!("average_badge_team1 >= {}", min_average_badge));
    }
    if let Some(max_average_badge) = settings.max_average_badge {
        where_clauses.push(format!("average_badge_team0 <= {}", max_average_badge));
        where_clauses.push(format!("average_badge_team1 <= {}", max_average_badge));
    }
    ...
    if let Some(is_low_pri_pool) = settings.is_low_pri_pool {
        where_clauses.push(format!("low_pri_pool = {}", is_low_pri_pool));
    }
    if let Some(is_new_player_pool) = settings.is_new_player_pool {
        where_clauses.push(format!("new_player_pool = {}", is_new_player_pool));
    }
