...
(
    ...
    comment text constraint api_keys_pk unique,
...
insert into public.api_keys (
    key, comment, data_access, disabled, esports_ingest
)
values ('fffd6bfd-2be9-4b7e-ab76-a9d1dca19b64', 'Test Key', true, false, false);

create table public.api_key_limits
(
    key         uuid                                  not null constraint limits_key_fkey references public.api_keys,
    path        text                                  not null,
...
insert into public.api_key_limits (
    key, path, rate_limit, rate_period
)
